...
#include "H5MD.hpp"  // keep python.hpp on top
...
namespace espressopp {
  ...
  void H5MD::read(std::string filename) {};
  void H5MD::detect_fs(std::string filename) {};
  void H5MD::print_number_particles() {};
  void H5MD::print_number_timesteps() {};
  void H5MD::print_pids_position(bool sorted) {};
  void H5MD::print_pids_velocity(bool sorted) {};
  void H5MD::print_pids(bool sorted) {};




   void H5MD::write_n_to_1(shared_ptr<System> system,
    	...
    	char *ch_f_name = new char[_file_name.length() + 1];
    	strcpy(ch_f_name, _file_name.c_str());
    ...
    void H5MD::write_n_to_n(shared_ptr<System> system,
            shared_ptr<integrator::MDIntegrator> _integrator,
            std::string _file_name,
    	...
    	size_t filename_length = _file_name.length();
    	string suffix = _file_name.substr(filename_length-3, 3);
    	string base_filename = _file_name.substr(0,filename_length-3);
    	string rankstring = static_cast<ostringstream*>( &(ostringstream() << rank) )->str();
		...
		double timestep = _integrator->getTimeStep();
		long long step = _integrator->getStep();
    ...
    void H5MD::write(shared_ptr<System> system,
            shared_ptr<integrator::MDIntegrator> _integrator,
            std::string _file_name,
			int _iomode,
            boost::python::list _data_to_store,
    	...
    	if (_iomode == 1 || _iomode == 0) {

			if (system->comm->rank() == 0  && !_append) {
				FileBackup backup(_file_name);
			} else {

				write_n_to_1(shared_ptr<System> system,
			            shared_ptr<integrator::MDIntegrator> _integrator,
			            std::string _file_name,
						int _iomode,
			            boost::python::list _data_to_store,
			...
			if (!_append) {
				int rank = system->comm->rank();
				size_t filename_length = _file_name.length();
				std::string suffix = _file_name.substr(filename_length-3, 3);
				std::string base_filename = _file_name.substr(0,filename_length-3);
				...
				write_n_to_n(shared_ptr<System> system,
			            shared_ptr<integrator::MDIntegrator> _integrator,
			            std::string _file_name,
						int _iomode,
			            boost::python::list _data_to_store,
    ...
    // Python wrapping
    void H5MD::registerPython() {

      using namespace espressopp::python;

      class_<H5MD, bases<ParticleAccess>, boost::noncopyable >
      ("io_H5MD", init< shared_ptr< System >,
                           ...
                           std::string ,
                           bool>())
//        .add_property("filename", &H5MD::getFilename,
//                                  &H5MD::setFilename)
//		.add_property("iomode", &H5MD::getIomode,
//										&H5MD::setIomode)
//        .add_property("unfolded", &H5MD::getUnfolded,
//                                  &H5MD::setUnfolded)
//        .add_property("length_factor", &H5MD::getLengthFactor,
//                                       &H5MD::setLengthFactor)
//        .add_property("length_unit", &H5MD::getLengthUnit,
//                                     &H5MD::setLengthUnit)
//        .add_property("append", &H5MD::getAppend,
//                                  &H5MD::setAppend)
        .def("write", &H5MD::write)
		.def("read", &H5MD::write)
      ;
    }
  }
